                                    ...
                                    @if ($approvalRoute && $item->approval_level == $approvalRoute->sequence - 1 && $item->status !== 'Rejected')
                                        @if (!($item->approval_level > 0))
                                            <form action="{{ route('items.approve', $item->id) }}" method="POST"
                                                ...
                                                @csrf
                                                <button type="submit"
                                                    class="btn btn-sm rounded-lg btn-outline-success btn-approve"
                                        ...
                                        @else
                                            ...
                                            <div class="dropdown dropstart">
                                                <button
                                                    class="btn btn-sm rounded-lg btn-outline-success dropdown-toggle h-100"
                                            ...
                                            @csrf
                                            <button type="button" class="dropdown-item h-100 btn-revisi text-warning">Revisi</button>
                                            </form>
                                            </li> --}}
                                                    <li>
                                                        <form action="{{ route('items.reject', $item->id) }}"
                                    ...
                                    @if ($canModify)
                                        ...
                                        @if (auth()->user()->hasMenuPermission($menu->id, 'destroy'))
                                            <form action="{{ route('items.destroy', $item->id) }}" method="POST"
                                                ...
                                                @method('DELETE')
                                                <button type="button"
                                                    class="btn btn-sm rounded-lg h-100 btn-outline-danger btn-delete"
