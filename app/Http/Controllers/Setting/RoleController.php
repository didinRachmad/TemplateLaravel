...
class RoleController extends Controller
{
    ...
    public function index()
    {
        ...
        $roles = Role::all();
        return view('setting.roles.index', compact('roles', 'menu'));
    }
    ...
    public function menuPermissions(Role $role)
    {
        ...
        $roleMenuPermissions = $role->menus->mapToGroups(function ($menu) {
            return [$menu->id => $menu->pivot->permission_id];
        })->toArray();

        // Ambil semua permission yang ada di sistem
        $permissions = Permission::all();

        return view('setting.roles.menu-permissions', compact('role', 'menus', 'roleMenuPermissions', 'permissions'));
    }
